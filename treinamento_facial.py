...
for arquivo in glob.glob(os.path.join("fotos/treinamento", "*.jpg")):
	...
	for face in facesDetectadas:
		...
		npArrayDescritorFacial = np.asarray(listaDescritorFacial, 
																				dtype=np.float64)
		...
		if descritoresFaciais is None:
			...
			descritoresFaciais = np.concatenate((descritoresFaciais,
																			npArrayDescritorFacial), axis = 0)
		indice[idx] = arquivo
		idx += 1
		
#print("Tamanho: {}\nFormato: {}".format(len(descritoresFaciais),
#																				descritoresFaciais.shape))
